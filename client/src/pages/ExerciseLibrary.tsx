...
export function ExerciseBrowser({ data, selectedWorkout, onAdd, onFavorite, showHeader = true }: { data: BellaData; selectedWorkout?: Workout; onAdd?: (exercise: ExerciseDefinition) => void; onFavorite: (id: string) => void; showHeader?: boolean }) {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("bella-fit:select-exercise", openExercise);
  }, [all]);
  const results = all.filter((exercise) => `${exercise.name} ${exercise.muscle} ${exercise.equipment}`.toLocaleLowerCase("pt-BR").includes(query.trim().toLocaleLowerCase("pt-BR")) && (muscle === "Todos" || exercise.muscle === muscle) && (equip === "Todos" || exercise.equipment === equip) && (!onlyFavorites || data.favorites.includes(exercise.id)));
  function createCustom(submitEvent: React.FormEvent) {
    submitEvent.preventDefault(); setCustomError("");
    if (!customName.trim()) { setCustomError("Digite um nome para o exercício."); return; }
    const definition: ExerciseDefinition = { id: makeId("custom"), name: customName.trim(), muscle: customMuscle, equipment: customEquipment, description: customDesc.trim() || "Exercício personalizado criado por você.", instructions: "Ajuste a execução de acordo com sua orientação profissional.", defaultSets: Math.min(10,Math.max(1,Number(customSets)||1)), defaultReps: customReps || "12", defaultRestSeconds: Math.max(0,Number(customRest)||0), custom: true };
    window.dispatchEvent(new CustomEvent("bella-fit:add-custom-exercise", { detail: definition }));
