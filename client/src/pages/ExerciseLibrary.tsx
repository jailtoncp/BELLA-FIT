...
export function ExerciseBrowser({ data, selectedWorkout, onAdd, onFavorite, showHeader = true, topContent, accountId }: { data: BellaData; selectedWorkout?: Workout; onAdd?: (exercise: ExerciseDefinition) => void; onFavorite: (id: string) => void; showHeader?: boolean; topContent?: ReactNode; accountId?: string }) {
  ...
  const results = all.filter((exercise) => `${exercise.name} ${exercise.muscle} ${exercise.equipment} ${exercise.purpose ?? ""} ${exercise.description} ${exercise.instructions} ${(exercise.primaryMuscles ?? []).join(" ")} ${(exercise.secondaryMuscles ?? []).join(" ")}`.toLocaleLowerCase("pt-BR").includes(query.trim().toLocaleLowerCase("pt-BR")) && (muscle === "Todos" || exercise.muscle === muscle) && (equip === "Todos" || exercise.equipment === equip) && (!onlyFavorites || data.favorites.includes(exercise.id)));
  function createCustom(submitEvent: React.FormEvent) {
    ...
    setCustomOpen(false); setCustomName(""); setCustomDesc(""); setCustomPurpose(""); setCustomPrimaryMuscles(""); setCustomSecondaryMuscles("");
  }
  return <div className="library-browser">
    ...
    {results.length ? <div className="exercise-library-grid">{results.map((exercise,index)=><Card className="exercise-tile" key={exercise.id} style={{animationDelay:`${Math.min(index,8)*25}ms`}}>
      ...
      {accountId&&<><input ref={(element)=>{gifInputs.current[exercise.id]=element}} type="file" accept="image/gif,.gif" aria-label={`Escolher GIF para ${exercise.name}`} className="exercise-gif-input" onChange={(event)=>void uploadGif(exercise,event.currentTarget.files?.[0])}/><button type="button" className="exercise-gif-picker" aria-label={`${gifUrls[exercise.id]||exercise.imageUrl?"Trocar":"Adicionar"} GIF de ${exercise.name}`} title={`${gifUploading===exercise.id?"Salvando":"Editar GIF"} de ${exercise.name}`} disabled={gifUploading===exercise.id} onClick={()=>gifInputs.current[exercise.id]?.click()}>{gifUploading===exercise.id?<LoaderCircle size={14}/>:<Pencil size={14}/>}</button></>}
